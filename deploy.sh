...
echo -e "${YELLOW}=== Production 무중단 배포 시작 ===${NC}"
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    echo -e "${YELLOW}환경 설정 파일을 생성해주세요.${NC}"
    exit 1
fi
...
# 기존 컨테이너가 있다면 완전히 제거 (ContainerConfig 오류 방지)
...
if ! sudo nginx -t; then
    echo -e "${RED}❌ Nginx 설정 오류! 배포 중단 및 롤백.${NC}"
    echo -e "${YELLOW}Nginx 설정 오류 상세:${NC}"
...
if sudo nginx -s reload; then
    ...
    echo -e "${RED}❌ Nginx reload 실패! 배포 중단 및 롤백.${NC}"
    if [ -f "$NGINX_CONFIG_BACKUP" ]; then
        echo -e "${YELLOW}백업된 Nginx 설정으로 복원 중...${NC}"
        sudo cp "$NGINX_CONFIG_BACKUP" "$NGINX_CONFIG"
        sudo nginx -t && sudo nginx -s reload
