-- Store 테이블의 store_id 컬럼을 id로 변경
-- 주의: 이 작업은 외래키 제약조건이 있는 경우 실패할 수 있습니다.
-- 먼저 외래키를 제거하고, 컬럼명을 변경한 후 외래키를 다시 추가해야 합니다.

-- 1. 외래키 제약조건 확인 및 제거 (필요한 경우)
-- SHOW CREATE TABLE Store; -- 외래키 제약조건 확인

-- 2. store_id 컬럼을 id로 변경
ALTER TABLE Store CHANGE COLUMN store_id id INT AUTO_INCREMENT;

-- 3. 외래키 제약조건이 있었다면 다시 추가
-- 예시:
-- ALTER TABLE 다른테이블 ADD CONSTRAINT fk_다른테이블_store 
