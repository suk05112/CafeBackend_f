...
테스트 전략:
    - validity를 과거 날짜로 직접 설정한 테스트 데이터 사용
    - clock.freeze_time()으로 현재 시간 고정
    - expire_gifticons() 배치 함수를 직접 호출 (스케줄러 미경유)
    - 페이레터 API는 unittest.mock으로 대체 (성공/실패 시나리오 각각 검증)
...
def setup_test_data(validity: date) -> tuple[int, int]:
...
def test_expired_payletter_success():
    """validity 당일, 페이레터 성공 → REFUNDED + refund COMPLETED + 90%."""
    ...
    try:
        with patch("core.scheduler._payletter_cancel", return_value=True):
            expire_gifticons()
        state = get_state(order_id, gid)
        print_result("페이레터 성공 → gifticon REFUNDED",
                     state["gifticon"] == "REFUNDED", f"gifticon={state['gifticon']}")
        print_result("페이레터 성공 → order REFUNDED",
                     state["order"] == "REFUNDED", f"order={state['order']}")
        print_result("페이레터 성공 → refund COMPLETED",
                     state["refund_status"] == "COMPLETED", f"refund={state['refund_status']}")
        print_result("환불금액 90% = 4500원",
                     state["refund_amount"] == 4500, f"amount={state['refund_amount']}")
    finally:
        teardown_test_data(order_id, gid)


def test_expired_payletter_failure():
    """페이레터 실패 → gifticon EXPIRED + refund FAILED (다음 배치 재시도 대상)."""
    today = clock.now().date()
    order_id, gid = setup_test_data(today)
    try:
        with patch("core.scheduler._payletter_cancel", return_value=False):
            expire_gifticons()
        state = get_state(order_id, gid)
        print_result("페이레터 실패 → gifticon EXPIRED",
                     state["gifticon"] == "EXPIRED", f"gifticon={state['gifticon']}")
        print_result("페이레터 실패 → refund FAILED",
                     state["refund_status"] == "FAILED", f"refund={state['refund_status']}")
    finally:
        teardown_test_data(order_id, gid)


def test_no_duplicate_on_completed_refund():
    """COMPLETED refund가 있으면 배치가 다시 처리하지 않음."""
    today = clock.now().date()
    order_id, gid = setup_test_data(today)
    ...
    try:
        with patch("core.scheduler._payletter_cancel") as mock_pg:
            expire_gifticons()
            print_result("COMPLETED refund 존재 시 페이레터 미호출",
                         mock_pg.call_count == 0, f"call_count={mock_pg.call_count}")
    finally:
        teardown_test_data(order_id, gid)
...
def main():
    print("\n=== GNB-174: 기프티콘 만료 배치 테스트 ===\n")

    print("1. validity 미래 → 만료 안됨")
    test_not_expired_before_validity()

    print("\n2. validity 당일 + 페이레터 성공 → REFUNDED")
    test_expired_payletter_success()

    print("\n3. validity 당일 + 페이레터 실패 → EXPIRED + FAILED")
    test_expired_payletter_failure()

    print("\n4. COMPLETED refund 존재 시 중복 처리 방지")
    test_no_duplicate_on_completed_refund()

    print("\n5. freeze_time 경계값 (2030-01-10 만료일)")
    test_time_boundary_freeze()

    print("\n6. 스케줄러 잡 1분 interval 등록 확인")
